...
ptFile   = [];%fullfile(dataDir,'mat/plumeTrack_output.mat'); % plumeTracker output (plot mask outlines)
geomFile = fullfile(dataDir,'thermCubeAnalysis/geometry.mat'); % plots axes/grid in projected meters
% geomFile = '';
ptCalcs  = '';
specFile = fullfile(dataDir,'spectral-calcs/specT_1D_2019-06-26_1213_w12_o8_n572.mat');
% thermFile = fullfile(dataDir,'spectral-calcs/thermStats_2019-07-05_n598.mat');
...
oFile = fullfile(dataDir,'vids/24A_all_diagnostics'); % leave the extension
% ---- THERMAL PROPS ----
% Idx = [390:2:1573]; % Indices to include (empty for all)
Idx = [430:2:1573];
% Idx = [754 1488];
...
fs = 12;
cbar_location = 'north';
cbar_position = [0.5 0.88 0.32 0.035];
...
% inset(1).x = T.VidTime;
% inset(1).y = T.H;
% inset(1).pos = [0.06 0.81 0.3 0.16];
...
% inset(2).pos = [0.06 0.65 0.3 0.16];
...
% fluxZ = [38 175 310]; % Heights for flux profiles
...
% inset(4).pos = [0.06 0.49 0.3 0.16];
% inset(4).xlab = '';
...
% inset(5).pos = [0.06 0.33 0.3 0.16];
% inset(5).xlab = 't [s]';
% inset(5).ylab = 'Integrated T [K]';
% inset(5).XTick = true;
% inset(5).YTick = true;
...
% inset(4).dat = 'T0';
% inset(4).x = T0.t; %,{'fluxMask',@(x) sParam.fluxX(x)}}; 
% inset(4).y = T0.prctile; %,{'fluxMask',@(x,y) sParam.fluxX(x)}};
% inset(4).pos = [0.06 0.49 0.3 0.16];
% inset(4).xlab = '';
% inset(4).ylab = 'T max [K]';
% inset(4).XTick = false;
...
for ii = ivec
    ...
    % Hack for now
%     fname = ['int_' Tout.File{num2str(idx)}];
...
%     set(fig, 'Position', [100 100 1*size(Frame,2) 1*size(Frame,1)])
    ...
    set(axa,'position',[(figw-w)/figw (figh-h)/h w/figw h/figh])
