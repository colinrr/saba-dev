...
% EVENT 24B (May, 11:32am local, explosion through steam cloud)
% =========================== USER INPUT =============================
...
asc_params = fullfile(ascDir,'AA052407_conv_1807.txt');
% asc_params = [];
...
deb = 17; % Starting image? Need not be the same as reference image
          % >> USE VECTOR to ignore fin and dN, and use indices in vector
          % instead -> STILL IMPLEMENTING
fin = 999; %874; % Leave empty to grab all files to end
...
refPix = [712 278]; % pixel coords in ref image corresponding to refLLE
