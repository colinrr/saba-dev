...
n = 1000;
t1 = linspace(0,500,n); % Seconds, for now
...
for ii = 1:length(alpha)
    for jj = 1:length(R)
        ...
        r = R0+Ve*t1;
        m_t = m0 + 4*pi*rho_a*Ve.^3*t1.^3;

        % ODE - Gamma(t) 
        % 
        % % timespan = linspace(0,10000,n);

        [t,T2] = ode45(@mixMe,t1,T0);
...
%% Plot this particular brand of bullshit
        ...
        axes(axd)
        plot(m_t/m0,T2/T0,'LineWidth',1.5);
        ylabel('$\Delta T/\Delta T_0$ [K]','interpreter','latex')
        xlabel('Mass/Mass_0')
%         set(gca,'YScale','log')
%         set(gca,'XTickLabel',[],'FontSize',12)
