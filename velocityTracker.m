...
muWinHeight = 36; % Height of windows for mean image profile

winHeight  = 36; % Height of windows to retrieve thermal stats (px)
...
% 2 parameters below are RELATIVE to winHeight:
% (ie detectionWindowStart  = winHeight * detection_window_scale + srcIdxM)
% (ie detectionWindowHeight = winHeight * detection_window_scale)
detection_window_offset = 0.5; % Vertical shift of detection window
detection_window_scale  = 0.5; % Vertical scale of detection window
...
det_zI1 = srcIdxM+round(detection_window_offset.*winHeight)-1; % Bottom height of detection window
det_zI = repmat((det_zI1+1:det_zI1+round(winHeight*detection_window_scale))', [1 Pwin]); % Set source region

% det_zI = src_zI(detection_window_offset:end,:);            % Set detection region
...
V0 = getThermStats(V.Vz,D.mask,D.z,D.t,src_zI,src_tI);
% [V0f,V0spec] = filterVelocities(V0,dt,'low',3,4,true);

% Detection window stats
Td = getThermStats(D.T,D.mask,D.z,D.t,det_zI,src_tI);
Vd = getThermStats(V.Vz,D.mask,D.z,D.t,det_zI,src_tI);
[Vdf,Vdspec] = filterVelocities(Vd,dt,'low',2,4,filter_plotflag);
% Tmax = squeeze(max(D.T.*D.mask,[],2));

% Tvar0 = zeros(1,P);
% HistC0 = zeros(P,numel(Tedges)-1);
% for pp=1:P
%     Ttemp = srcT(:,:,pp);
%     Tvar0(pp) = nanvar(Ttemp(:));
%     [HistC0(pp,:),~] = histcounts(Ttemp,Tedges);
% end    
% HistC0 = (HistC0./max(HistC0,[],2))';
% Tvar  = squeeze(var(nanvar(srcT,[],2),[],1));
% Get velocities and calc displacements?

%% Implement detection here
disp('Running source pulse detection...')
[tTrig,yTrig] = thermPulseDetection(Td, det_field, prefilt_params, det_params, detection_plotflag);
...
Vpos = trackVelocities(V.Vz,D.mask,D.T,trk_zI(:,1),src_tI,winI,D.z,D.t); 

%% Get stats of tracked thermal features
disp('Retrieving thermal profiles...')

for kk=numel(winI):-1:1
...
title(dax(1),sprintf('Detection window, Win Height: %.1f m, Win Dur: %.1f s, Win OL: %.2f'...
    ,round(winHeight.*detection_window_scale)*-D.dz,winDur*dt,tOver/winDur))
